...
function Toggle({ on, onToggle, color = "#16a34a" }: { on: boolean; onToggle: () => void; color?: string }) {
  return (
      ...
      <div className="relative w-full bg-white rounded-t-3xl px-5 pt-5 pb-8 max-h-[85vh] overflow-y-auto animate-[slideIn_0.35s_ease]">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-green-50 rounded-xl flex items-center justify-center">{icon}</div>
...
function PasswordSheet({ onClose }: { onClose: () => void }) {
  ...
  const handleSave = () => {
    ...
    if (nw.length < 6) { setError("La nueva contraseña debe tener mínimo 6 caracteres."); return; }
    if (nw !== conf) { setError("Las contraseñas no coinciden."); return; }
    setError(""); setSaved(true); setTimeout(onClose, 1500);
  };
  ...
  const Field = ({ label, val, setVal, show, setShow }: any) => (
    <div className="mb-3">
      ...
      <div className="flex items-center gap-2 bg-gray-50 border-2 border-gray-100 rounded-xl px-3 focus-within:border-green-500 focus-within:bg-white transition-all">
  ...
  return (
    <SheetWrapper title="Modificar contraseña" icon={<Lock className="w-5 h-5 text-green-600" />} onClose={onClose}>
      ...
      <button onClick={handleSave}
        ...
        style={{ background: "linear-gradient(135deg, #14532d, #16a34a)", boxShadow: "0 6px 18px rgba(22,163,74,0.3)" }}>
        Guardar contraseña
      </button>
    </SheetWrapper>
  );
...
function NotificationsSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    <SheetWrapper title="Alertas y notificaciones" icon={<Bell className="w-5 h-5 text-green-600" />} onClose={onClose}>
      <div className="space-y-3 mb-4">
        {items.map((item) => (
          <div key={item.key} className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
            <div className="w-8 h-8 rounded-lg bg-green-50 text-green-600 flex items-center justify-center">{item.icon}</div>
            <div className="flex-1">
              <p className="text-sm font-bold text-gray-800">{item.label}</p>
              <p className="text-xs text-gray-500">{item.sub}</p>
            </div>
            <Toggle on={notifPrefs[item.key]} onToggle={() => toggleNotifPref(item.key)} color="#16a34a" />
          </div>
        ))}
      </div>
      <div className="bg-green-50 rounded-xl px-4 py-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-xs font-bold text-green-800">Sonido de notificaciones</p>
        </div>
        <p className="text-xs text-green-600">Activado · Sonido por defecto</p>
      </div>
    </SheetWrapper>
  );
}
...
function LanguageSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    <SheetWrapper title="Cambiar idioma" icon={<Globe className="w-5 h-5 text-green-600" />} onClose={onClose}>
  ...
  const sections = [
    {
      ...
      items: [
        { icon: <Lock className="w-4 h-4" />, label: "Modificar contraseña", sub: "Cambia tu clave de acceso", color: "#dc2626", action: () => setSheet("password") },
        { icon: <ShieldCheck className="w-4 h-4" />, label: "Privacidad", sub: "Controla tu información visible", color: "#0891b2", action: () => navigate("/worker/privacy") },
      ],
    },
    {
      title: "Preferencias",
      items: [
        { icon: <Bell className="w-4 h-4" />, label: "Alertas y notificaciones", sub: "Push, email y SMS", color: "#16a34a", action: () => setSheet("notifications") },
        { icon: <Palette className="w-4 h-4" />, label: "Color del tema", sub: "Personaliza los colores de la app", color: "var(--color-primary)", action: () => setSheet("theme") },
        { icon: darkMode ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />, label: "Modo oscuro", sub: darkMode ? "Actualmente: Oscuro" : "Actualmente: Claro", color: "#d97706", action: toggleDarkMode, toggle: true },
        { icon: <Globe className="w-4 h-4" />, label: "Idioma", sub: "Español (Latinoamérica)", color: "#7c3aed", action: () => setSheet("language") },
      ],
    },
    {
      title: "Soporte",
      items: [
        { icon: <HelpCircle className="w-4 h-4" />, label: "Centro de ayuda", sub: "Preguntas frecuentes", color: "#059669", action: () => {} },
        { icon: <MessageSquare className="w-4 h-4" />, label: "Contactar soporte", sub: "Escríbenos por chat", color: "#0284c7", action: () => {} },
      ],
    },
  ];
      ...
      <div className="px-4 py-4 space-y-5">
        {sections.map((sec) => (
          <section key={sec.title}>
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1 mb-2">{sec.title}</p>
